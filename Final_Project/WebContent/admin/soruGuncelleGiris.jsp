...
try {
	...
	if(!con.isClosed()){ 	
		String sorgu="select * from soru where soru_id="+Integer.parseInt(request.getParameter("s_id"))+"";
		session.setAttribute("s_id", request.getParameter("s_id"));
		pst = con.prepareStatement(sorgu);
	    rs = pst.executeQuery();
	    %>
<form action="soruGuncelle.jsp" method="post">
	<table id="customers">
	  <tr>
	    <th colspan="2">Soru Guncelleme</th>
	  </tr>
	<% while(rs.next()){ %>
		  <tr>
		    <td>Soru Text:</td>
		    <td><input type="text" value="<%= rs.getString("text") %>" name="txtsoru"></input></td>
		  </tr>
		    <tr>
		    <td>A</td>
		    <td><input type="text" value="<%= rs.getString("a") %>" name="a"></input></td>
		  </tr>
		    <tr>
		    <td>B:</td>
		    <td><input type="text" value="<%= rs.getString("b") %>" name="b"></input></td>
		  </tr>
		    <tr>
		    <td>C:</td>
		    <td><input type="text" value="<%= rs.getString("c") %>" name="c"></input></td>
		  </tr>
		    <tr>
		    <td>D:</td>
		    <td><input type="text" value="<%= rs.getString("d") %>" name="d"></input></td>
		  </tr>
		  <tr><td colspan="2" align="center"><input type="submit" value="Guncelle"></input></td></tr>
	  <%}
	  %>
	  </table>
	 </form>
