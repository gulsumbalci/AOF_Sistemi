...
try {
	...
	if(!con.isClosed()){ 	
		...
		preStmt = con.prepareStatement("select * from soru");
	    rs = preStmt.executeQuery();
	    %>
	    <table id="customers">
	    <tr><th colspan="8">TUM SORULAR</th></tr>
		  <tr>
		    <th>Id</th>
		    <th>Soru Text</th>
		    <th>A</th>
	    ...
	    <%  while (rs.next()) { %>
	    <tr>
		    <td><%=rs.getInt("soru_id")%></td>
		    <td ><%=rs.getString("text")%></td>
		    <td ><%=rs.getString("a")%></td>
		    <td ><%=rs.getString("b")%></td>
		    <td ><%=rs.getString("c")%></td>
		    <td ><%=rs.getString("d")%></td>
		    <td><a href='soruSil.jsp?s_id=<%=rs.getInt("soru_id")%>'>Soru Sil</a></td>
		    <td><a href='soruGuncelleGiris.jsp?s_id=<%=rs.getInt("soru_id")%>'>Soru Guncelle</a></td>
	        </tr> <% }//end while %>
